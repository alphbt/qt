...
class SVec3f
{
 public:
   ...
   {
     if(fabs(x) >= EPS_PRECISION || fabs(y) >= EPS_PRECISION)
     {
       return SVec3f(-y, x, 0);
     }
     return SVec3f(-z, 0, x);
...
public: 
  ...
  Complex_SVec3f() : re(0, 0, 0), im(0, 0, 0) { }
...
inline std :: ostream& operator<<(std :: ostream& o, const Complex_SVec3f& vec)
{
  o << vec.Re() << " + i * " << vec.Im();
  return o;
}
...
public: 
  ...
  SVec2f GetProjectionPlane(const SVec3f& vec) const;
  const SVec3f& get_normal() const { return norm; }
};
